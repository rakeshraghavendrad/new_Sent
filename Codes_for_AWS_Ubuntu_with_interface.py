...
class App(QWidget):
    ...
    def initUI(self):
        ...
        self.button = QPushButton('Proceed', self)
    ...
    @pyqtSlot()
    def on_click(self):
        self.reply = QMessageBox.question(self, 'Sentiment Analysis analysis', 'Please click YES to continue OR NO to return to main menu', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if self.reply == QMessageBox.Yes:
            self.reply = QMessageBox.question(self, 'Video to text analysis', 'Please click YES to Download videos from s3 bucket OR NO to return to main menu', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if self.reply == QMessageBox.Yes:
                self.ACCESS_KEY = QInputDialog.getText(self, 'Input Dialog', 'INPUT ACCESS KEY IN DOUBLE QUOTES')
                self.SECRET_KEY = QInputDialog.getText(self, 'Input Dialog', 'INPUT SECRET KEY IN DOUBLE QUOTES')
                path = '/home/ubuntu/new_Sent/videos/'
                path1 = "/home/ubuntu/new_Sent/aud/"
                self.reply = QMessageBox.question(self, 'Change the Link to get filenames', 'The current link is : http://dev.ibridge360.com/api/UserAnswersAndScores/getFilenames do you like to change?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                if (self.reply == QMessageBox.Yes):
                    self.Filename = QInputDialog.getText(self, 'Input Dialog', 'INPUT THE NEW LINK IN DOUBLE QUOTES')
                    get_File_names = self.Filename
                else :
                     get_File_names = 'http://dev.ibridge360.com/api/UserAnswersAndScores/getFilenames'
                
                self.reply = QMessageBox.question(self, 'Change the Link to add sentiment data', 'The current link is : http://dev.ibridge360.com/api/UserAnswersAndScores/addSentimentData do you like to change?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)        
                if (self.reply == QMessageBox.Yes):
                    self.populatename = QInputDialog.getText(self, 'Input Dialog', 'INPUT THE NEW LINK IN DOUBLE QUOTES')
                    populate_DB = self.populatename
                else:
                    populate_DB = 'http://dev.ibridge360.com/api/UserAnswersAndScores/addSentimentData'
                ...
                ACCESS_KEY = self.ACCESS_KEY     #'AKIAWOFGABKXMIV6HO23'
                SECRET_KEY = self.SECRET_KEY     #'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'
                    ...
                    if filename.endswith(".mp4"):
                        ...
                        if (b >2.00):
                            ...
                            try:
                                ...
                                data_e.to_csv("Analysis_ERROR_data.csv",mode='a')
                ...
                try:
                    ...
                    if(os.path.exists(file2) and os.path.isfile(file2)):
                        ...
                        self.reply = QMessageBox.question(self, 'Message Box', 'program completed successfully and Populated values in databases', QMessageBox.Yes)
                except:   
                    df = pd.read_csv('Analysis_AWS_data.csv')
                    du1=df.drop_duplicates()
                    df1 = du1.loc[:, ~du1.columns.str.contains('^Unnamed')]
            ...
            self.reply = QMessageBox.question(self, 'Sentiment Analysis analysis', 'returning to main menu', QMessageBox.Yes)
